...
app = Flask('')
@app.route('/')
def home(): return "Bot is Online & Fixed! ⚡"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🎮 *Gaming Arena*\n\n/game - Start X-O\n/rps - Start RPS", parse_mode=constants.ParseMode.MARKDOWN)
...
async def rps_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🥊 *Rock Paper Scissors*\nBy: {update.effective_user.first_name}", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Join Match", callback_data=f"rj_{rid}")]]))

# --- CALLBACK HANDLER ---
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer() # Loading turant khatam
