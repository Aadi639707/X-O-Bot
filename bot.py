...
if MONGO_URL:
    try:
        ...
        stats_col = db['wins']
    except: logger.error("MongoDB Error")
...
def get_board_markup(board, gid):
    # API 8.0 Colorful styling logical handling
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "🎮 *X/O Gaming Arena* 🎮\n\n"
        "Welcome! Use the commands below to play:\n"
        "🚀 /game - Start a new match\n"
        "🏆 /leaderboard - Top Players\n"
        "📖 /help - How to play"
    )
    btns = [
        [InlineKeyboardButton("➕ Add to Group", url=f"https://t.me/{bot_user}?startgroup=true")],
        [InlineKeyboardButton("🎮 Play Now", callback_data="start_game_cmd")]
    ]
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("j_"):
        ...
        if win:
            ...
            next_n = g['n2'] if g['turn'] == 'O' else g['n1']
            await q.edit_message_text(f"🎮 *Match Live!*\n❌: {g['n1']}\n⭕: {g['n2']}\n\nTurn: {next_n} ({g['turn']})", reply_markup=get_board_markup(g['board'], gid), parse_mode=constants.ParseMode.MARKDOWN)
