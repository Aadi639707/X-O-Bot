    ...
    if data.startswith("lb_"):
        ...
        if action == "j":
            ...
            if win:
                ...
                save_win(uid, w_name)
                await context.bot.send_message(q.message.chat_id, f"🏆 *{w_name}* won the match!", parse_mode=constants.ParseMode.MARKDOWN)
                del games[gid]
            elif all(cell != " " for row in g['board'] for cell in row):
                await q.edit_message_reply_markup(reply_markup=get_board_markup(g['board'], gid))
                await context.bot.send_message(q.message.chat_id, "🤝 Draw!")
                del games[gid]
            else:
