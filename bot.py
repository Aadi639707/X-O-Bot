...
# --- WORD HUNT LOGIC (PRIORITY READER) ---
async def check_word(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_text == game['word']:
        ...
        await update.message.reply_text(f"🥳 *CORRECT!* {name} guessed `{game['word']}` and won 1 point!")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith("hint_"):
        ...
        if cid in active_hunts:
            ...
            else: await q.answer("❌ Hint limit reached!", show_alert=True)
        ...
        if rid in rps_games and rps_games[rid]['p1'] != uid and rps_games[rid]['p2'] is None:
            ...
            await q.edit_message_text(f"🥊 {g['n1']} vs {g['n2']}\nChoose your move!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🪨 Rock", callback_data=f"rm_{rid}_R"), InlineKeyboardButton("📄 Paper", callback_data=f"rm_{rid}_P"), InlineKeyboardButton("✂️ Scissors", callback_data=f"rm_{rid}_S")]]))
...
if __name__ == '__main__':
    ...
    bot.add_handler(CommandHandler("leaderboard",
